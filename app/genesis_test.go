...
func TestNewDefaultGenesisAccount(t *testing.T) {
	...
	require.Equal(t, sdk.NewInt(1000).MulRaw(assets.MicroUnit), acc.Coins.AmountOf(assets.MicroSDRDenom))
	require.Equal(t, sdk.TokensFromTendermintPower(100), acc.Coins.AmountOf(defaultBondDenom))
}

func TestGenesisStateSanitize(t *testing.T) {
	genesisState := makeGenesisState(t, nil)
	require.Nil(t, TerraValidateGenesisState(genesisState))
